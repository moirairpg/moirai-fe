...
const HIGH_LEVEL_COST = 2;

type AttributeAllocationOptions = {
  points?: number;
  levelCap?: number;
};

export function useAttributeAllocation(options?: AttributeAllocationOptions) {
  ...
  const highCostFrom = (creation?.levelCap ?? 0) + 1;

  const levelOf = (attribute: string) => pending[attribute] ?? 0;

  const costOfLevels = (level: number) =>
    Math.min(level, highCostFrom - 1) + Math.max(0, level - (highCostFrom - 1)) * HIGH_LEVEL_COST;

  const costOfNextLevel = (attribute: string) =>
    levelOf(attribute) + 1 >= highCostFrom ? HIGH_LEVEL_COST : 1;

  const spent = attributes.reduce((sum, attribute) => sum + costOfLevels(levelOf(attribute.name)), 0);
  ...
  const canIncrement = (attribute: string) =>
    creation !== null && levelOf(attribute) < levelCap && remaining >= costOfNextLevel(attribute);
